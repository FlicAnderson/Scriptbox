## Socotra Project :: script_herbSpxEorNA_geog-Socotra.R
# ======================================================== 
# (23rd July 2014)
# Author: Flic Anderson
#
# dependant on: "O://CMEP\ Projects/Scriptbox/function_livePadmeArabiaCon.R"
# saved at: O://CMEP\-Projects/Scriptbox/script_herbSpxEorNA_geog-Socotra.R
#
# AIM: count number of Edinburgh (E) herbarium specimen records or records where no record is specified (ie missing data/NA) from Socotra in Padme. Note: includes current dets only, and synonyms are included.
#
# --------------------------------------------------------
...
# 1) run query to find all herbarium specimens from Socotra which are Edinburgh (E) specimens, or have no herbarium listed
# 2) print number of specimens from E or where herbarium isn't listed in Padme
...
WHERE (Geog.fullName LIKE '%Socotra%' AND Dets.Current=True AND Herbaria.id IS NULL) OR (Geog.fullName LIKE '%Socotra%' AND Dets.Current=True AND Herbaria.id=7);"
...
# run the query
herbENA <- sqlQuery(con_livePadmeArabia, qry)

# sleep (2 seconds) to allow query to run
Sys.sleep(2)

# report number of specimens from Socotra which have no herbarium listed or are Edinburgh specimens from Padme records 
print(paste("Padme contains records for", nrow(herbENA), "specimens which are recorded as Edinburgh (E) specimens, or with no herbarium listed", sep=" "))
