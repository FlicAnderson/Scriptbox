...
# list all distinct taxa !no authority! - names with different authorities will be counted only once
# eg. Bellis perennis Flic, Bellis perennis L. -> "Bellis perennis" x1 count
# this is a slighly misleading figure, but gives a minimum number if many of the names have errors in authority taxonomy etc.
socotraTaxaNoAuth <- sqldf("SELECT DISTINCT taxon FROM allRex")
socotraTaxaNoAuth <- sort(socotraTaxaNoAuth[,], na.last=TRUE, decreasing=FALSE)


# list all distinct taxa from socotra (full species names with auth.)
socotraTaxa <- sqldf("SELECT DISTINCT taxonFull FROM allRex")
socotraTaxa <- sort(socotraTaxaFull[,], na.last=TRUE, decreasing=FALSE)

# report number of taxa found on Socotra from Padme records 
