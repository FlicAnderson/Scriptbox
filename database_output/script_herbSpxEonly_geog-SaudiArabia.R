## Saudi Arabia Project :: script_herbSpxEonly_geog-SaudiArabia.R
# ======================================================== 
# (15th January 2015)
# Author: Flic Anderson
#
# dependant on: "O://CMEP\ Projects/Scriptbox/function_livePadmeArabiaCon.R"
# saved at: O://CMEP\-Projects/Scriptbox/script_herbSpxEonly_geog-SaudiArabia.R
#
# AIM: count number of Edinburgh herbarium specimen records from Saudi Arabia in Padme. Note: includes current dets only, and synonyms are included.
#
# --------------------------------------------------------
...
source("database_connections/function_livePadmeArabiaCon.R")
...
WHERE Geog.fullName LIKE '%Saudi Arabia%' AND Dets.Current=True AND Herbaria.id=7;"

##WHERE Geog.fullName LIKE '%Saudi Arabia%' AND Dets.Current=True AND (Herbaria.id=7 OR HERBARIA.id IS NULL);"
...
# plot number of specimens at E by year
#plot(table(herbEonly$date1YYYY, useNA="ifany"), main = "Herbarium Specimens at E from Saudi Arabia", ylab="number of specimens", xlab="year entered", cex.axis=0.95)

# report table of number of collections by collector
#table(herbEonly$collector[, drop=TRUE], useNA="ifany")

# report table of collectors for a specific year (e.g. 1982)
#table(herbEonly[which(herbEonly$date1YYYY=="1982"),]$collector[, drop=TRUE], useNA="ifany")
